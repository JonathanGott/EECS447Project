...
$avgSql = "SELECT ROUND(AVG(RATING), 1) as avg_rating, COUNT(*) AS review_count FROM Review WHERE Movie_ID = ?";
...
$avgRating = $avgData["avg_rating"] ?? "No ratings yet";
$reviewCount = $avgData["review_count"];

//Fetch all reviews with usernames (JOIN QUERY)
...
//Handle success/error message from submit_review.php
$message = "";
$messageClass = "";
        ...
        <section class="section">
            <div class="details-card">
                ...
                <div>
                    ...
                    <p><?php echo htmlspecialchars($movie["Genre"]); ?> &bull; <?php echo htmlspecialchars($movie["Release_Year"]); ?> &bull; <?php echo htmlspecialchars($movie["Director"]); ?></p>

                    <?php if ($reviewCount > 0): ?>
                        <p class="average-rating">Average Rating: <?php echo $avgRating; ?> / 5 &nbsp;(<?php echo $reviewCount; ?> review<?php echo $reviewCount !== 1 ? "s" : ""; ?>)</p>
                    <?php else: ?>
                        <p class="average-rating">No ratings yet - be the first!</p>
                    <?php endif; ?>
                </div>

                <!--RIGHT Review form or message-->
                <div>
                    <?php if (!isset($_SESSION["user_id"])): ?>
                        <div class="review-form">
                            <h3>Rate this movie</h3>
                            <p style="color:#b8beca;">You must be <a href="login.php" style="color:#8fb4f5;">logged in</a> to leave a review.</p>
                        </div>
                ...
                <?php else: ?>
                    <?php while ($review = mysqli_fetch_assoc($reviewsResult)): ?>
                        <div class="review-card">
                            <p class="stars">Rating: <?php echo htmlspecialchars($review["Rating"]); ?> / 5</p>
                            <h3><?php echo htmlspecialchars($review["Username"]);?></h3>

                            <?php if (!empty($review["Review_Text"])): ?>
                                <p><?php echo htmlspecialchars($review["Review_Text"]); ?></p>
                            <?php endif; ?>
                            <p style="color:#9aa3b4; font-size:0.85em;"><?php echo date("M j, Y", strtotime($review["Review_Date"])); ?></p>
